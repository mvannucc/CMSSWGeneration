...
for ((i=1;i<=$nloops;i++)); 
do    
    echo "cmsRun -e -j FrameworkJobReport.xml gen_step.py jobNum="$1" nEvents="$nEventPerLoop" "$3" outputName=genStep_"$i".root "$5" "$6" "$7" "$8" "$9" generationStep="$i" jobEvents="$nevents
    cmsRun -e -j FrameworkJobReport.xml gen_step.py jobNum=$1 nEvents=$nEventPerLoop $3 outputName=genStep"_"$i.root $5 $6 $7 $8 $9 generationStep=$i jobEvents=$nevents 
